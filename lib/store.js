...
const EMPTY = {
  ...
  webhookLog: [],      // { id, at, event, refId, moduleId, org, status, url, ok, httpStatus, error } — capped partner-webhook delivery log
};

// Unbounded collections stored as per-record documents in Firestore.
const SUBCOLLECTIONS = ['users', 'enrollments', 'lessonProgress', 'notifications', 'outbox', 'sessions', 'webhookLog'];
...
function docIdFor(coll, rec) {
  switch (coll) {
    ...
    case 'webhookLog': return enc(rec.id);
